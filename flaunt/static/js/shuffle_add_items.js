window.Manipulator = (function($) {
  ...
  Manipulator.prototype.initShuffle = function() {
    ...
    this.filterOp = $('#option > ul > li > a');
  };

  Manipulator.prototype.setupEvents = function() {
    var self = this;

    $('#filterOp').on('click', $.proxy( self.onAddFilter, self ));
    ...
    self.$el.on('removed.shuffle', function( evt, $collection, shuffle ) {
      ...
      console.log(shuffle.prototype._filter());
      $.ajax({type: 'POST', url:'getproduct', data: {'cat': $collection['group'], 'csrfmiddlewaretoken': '{{ csrf_token  }}'}, success: function(data){
        console.log(data);
  ...
  Manipulator.prototype.onAddFilter = function( e ) {
    console.log(this);
